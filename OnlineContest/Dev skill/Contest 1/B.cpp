...
void input()
{
    ...
    {
        int k = 1;
        for(int i = 1; i <= n; i++) ar[ i ][ 0 ] = 0;
        for(int i = 1; i <= n; i++)
        {
            for(int j = 1; j <= k; j++) sf("%d", &ar[ i ][ j ]); ar[ i ][ 0 ] = k; k++;
        }
...
long long DP(int i, int j)
{
    if(i == n && j <= ar[i][0]) return ar[ i ][ j ];
    if(i > n) return 0;
    if(j > ar[ i ][ 0 ]) return 0;
    long long &ret = mamo[ i ][ j ];
    if(ret != -1) return ret;
    return ret = ar[ i ][ j ] + max(DP(i + 1, j), DP(i + 1, j + 1));
}
