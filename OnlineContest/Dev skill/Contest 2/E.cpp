...
void input()
{
    ...
    {
        sf("%d %d", &work, &man);
        for(int i = 0; i < man; i++) sf("%d %d", &in[i].xi, &in[i].yi);
        CLR(mamo, -1);
        pf("Case %d: %lld\n", ++kag, DP(0, work));
    }
}
lld DP(int pos, int remain)
{
    ...
    for(int i = in[pos].xi; i <= in[pos].yi; i++)
    {
        ret = ret + DP(pos + 1, remain - i);
        ret = ret % 1000000007;
    }
    return ret;
}
