...
struct RelationClassifier
{
    string isBijection(vector <int> domain, vector <int> range)
    {
        ...
        if(domain.size() != range.size()) ret = "Not";
        else if(domain.size() == 1 && range.size() == 1) ret = "Bijection";
        else
        {
        	sort(domain.begin(), domain.end());
        	sort(range.begin(), range.end()); int c = 0;
        	for(int i = 0; i < range.size(); i++)
        		if(domain[ i ] == range[ i ]) c++;
        	
			if(range.size() - c > 1) ret = "Not";
			else ret = "Bijection";
		}
